...
### مفهوم توان عملیاتی (Throughput) در شبکه‌های کامپیوتری

علاوه بر **تأخیر (delay)** و **از دست رفتن بسته‌ها (packet loss)**، یکی دیگر از معیارهای مهم عملکرد در شبکه‌های کامپیوتری، **توان عملیاتی سرتاسری (end-to-end throughput)** است.
برای تعریف توان عملیاتی، فرض کنید می‌خواهیم یک فایل بزرگ را از **میزبان A** به **میزبان B** در یک شبکهٔ کامپیوتری منتقل کنیم. این انتقال می‌تواند مثلاً ارسال یک ویدئو از یک کامپیوتر به کامپیوتر دیگر باشد.

**توان عملیاتی لحظه‌ای (instantaneous throughput)** در هر لحظه، نرخ دریافت فایل (بر حسب بیت بر ثانیه) در میزبان B است. بسیاری از برنامه‌های دانلود، این مقدار را در رابط کاربری خود نشان می‌دهند — شاید شما هم در هنگام دانلود این مقدار را مشاهده کرده باشید! (می‌توانید با استفاده از برنامه‌هایی مانند *Speedtest* میزان تأخیر و توان عملیاتی بین خود و سرورهای مختلف را اندازه‌گیری کنید.)

اگر فایل شامل **F بیت** باشد و کل انتقال آن **T ثانیه** طول بکشد، در آن صورت **توان عملیاتی متوسط (average throughput)** برابر خواهد بود با:

[
\text{Throughput} = \frac{F}{T} , \text{bps}
]

برای برخی کاربردها مانند **تلفن اینترنتی (Internet telephony)**، مطلوب است که **تأخیر کم** باشد و **توان عملیاتی لحظه‌ای** همیشه بالاتر از حداقل مقدار معینی باقی بماند (مثلاً بالای 24 kbps برای تماس صوتی یا 256 kbps برای ویدئوی زنده).
برای کاربردهایی مثل **انتقال فایل**، تأخیر اهمیت کمتری دارد ولی دستیابی به **بیشترین توان عملیاتی ممکن** مطلوب است.

---

### مثال ۱: دو لینک بین سرور و کاربر

در شکل 1.19(a)، دو سیستم انتهایی (سرور و کاربر) توسط دو لینک ارتباطی و یک روتر به هم متصل شده‌اند.
فرض کنید می‌خواهیم فایلی را از سرور به کاربر منتقل کنیم.
سرعت لینک بین سرور و روتر را **Rs** و سرعت لینک بین روتر و کاربر را **Rc** می‌نامیم.

اگر تنها داده‌ای که در شبکه جابجا می‌شود همین فایل باشد، سؤال این است:
**توان عملیاتی از سرور به کاربر چقدر است؟**

می‌توانیم بیت‌ها را مانند جریان مایع و لینک‌های ارتباطی را مانند لوله‌های انتقال در نظر بگیریم.
واضح است که:

* سرور نمی‌تواند بیت‌ها را سریع‌تر از **Rs** ارسال کند.
* روتر نیز نمی‌تواند بیت‌ها را سریع‌تر از **Rc** منتقل کند.

بنابراین:

* اگر **Rs ≤ Rc** باشد، بیت‌ها با نرخ Rs به کاربر می‌رسند → توان عملیاتی = **Rs**
* اگر **Rc ≤ Rs** باشد، روتر فقط می‌تواند با نرخ Rc داده را بفرستد → توان عملیاتی = **Rc**

در نتیجه، توان عملیاتی سرتاسری برابر است با:

[
\text{Throughput} = \min(R_s, R_c)
]

اگر اندازه فایل **F بیت** باشد، زمان تقریبی انتقال آن برابر است با:
...
برای مثال:
اگر یک فایل MP3 با اندازه ( F = 32 \text{ میلیون بیت} ) داشته باشیم،
سرور با نرخ ( R_s = 2 \text{ Mbps} ) ارسال کند،
و کاربر دارای لینک ( R_c = 1 \text{ Mbps} ) باشد،
آنگاه زمان لازم برای دریافت فایل تقریباً **32 ثانیه** خواهد بود.

(این محاسبات تقریبی هستند و تأخیرهای پردازش، بافر و پروتکل‌ها در نظر گرفته نشده‌اند.)

---

### مثال ۲: مسیر با N لینک

در شکل 1.19(b)، مسیر بین سرور و کاربر شامل **N لینک** است که نرخ انتقال آنها به‌ترتیب ( R_1, R_2, ..., R_N ) می‌باشد.
با همان منطق قبلی، توان عملیاتی کل مسیر برابر است با:

[
\text{Throughput} = \min(R_1, R_2, ..., R_N)
]

یعنی نرخ لینک گلوگاه (bottleneck link).

---

### مثال ۳: اینترنت امروزی با لینک‌های پرسرعت در هسته (Core)

در شکل 1.20(a)، سرور و کاربر از طریق شبکه‌ای به هم متصل هستند.
سرور با نرخ **Rs** و کاربر با نرخ **Rc** به شبکه متصل‌اند، در حالی‌که تمام لینک‌های بخش **هستهٔ شبکه (core)** دارای ظرفیت بسیار بالایی هستند.
در چنین شرایطی، چون هستهٔ شبکه گلوگاه نیست، **توان عملیاتی کلی** دوباره برابر است با:

[
\text{Throughput} = \min(R_s, R_c)
]

بنابراین در اینترنت امروزی، گلوگاه اصلی معمولاً در **شبکه‌های دسترسی (Access Network)** است، نه در هستهٔ اینترنت.

---

### مثال ۴: چندین جریان هم‌زمان (10 سرور و 10 کاربر)

در شکل 1.20(b)، ده سرور و ده کاربر به هستهٔ شبکه متصل‌اند و هر زوج سرور-کاربر در حال دانلود فایل هستند.
فرض کنید لینکی در بخش هسته وجود دارد که همهٔ این ده جریان از آن عبور می‌کنند و نرخ انتقال آن **R** است.
اگر سایر لینک‌ها ظرفیت زیادی داشته باشند، توان عملیاتی هر انتقال به‌صورت زیر تعیین می‌شود:

* اگر **R** بسیار بزرگ باشد (مثلاً ۱۰۰ برابر Rs و Rc)، آنگاه گلوگاه در دسترسی است و توان عملیاتی هر انتقال برابر با ( \min(R_s, R_c) ) خواهد بود.
* اگر **R** هم‌مرتبه با Rs و Rc باشد، آنگاه گلوگاه در لینک مشترک هسته قرار دارد.

برای مثال:
اگر ( R_s = 2 \text{ Mbps}, R_c = 1 \text{ Mbps}, R = 5 \text{ Mbps} )
و این لینک ۵ مگابیتی به طور مساوی میان ۱۰ انتقال تقسیم شود،
در نتیجه هر جریان تنها ( 500 \text{ kbps} ) پهنای باند دریافت می‌کند.
پس توان عملیاتی هر دانلود برابر است با **500 kbps**.

---

### جمع‌بندی

نمونه‌های شکل‌های 1.19 و 1.20 نشان می‌دهند که **توان عملیاتی** به نرخ انتقال لینک‌هایی که داده از آنها عبور می‌کند بستگی دارد.
وقتی ترافیک دیگری وجود ندارد، توان عملیاتی تقریباً برابر با **حداقل نرخ انتقال در مسیر** است.
اما در شرایط عمومی‌تر، توان عملیاتی علاوه بر نرخ لینک‌ها، به **ترافیک عبوری دیگر** نیز وابسته است.
حتی اگر لینکی دارای ظرفیت بالا باشد، اگر چندین جریان داده از آن عبور کنند، ممکن است همان لینک به **گلوگاه (bottleneck)** تبدیل شود.
